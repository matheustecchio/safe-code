...
The workflow contains one bounded recovery mode only for original run `35912409247`, attempt `1`, source commit `6d963b58d8fce12511538fa86548b2e77b3bce21`, bundle artifact `10773757838`, attempt-receipt artifact `10772978663`, and VSIX SHA-256 `914c44d330bea2ed54fc54b90ec88b94f4092535d7b0b6864b9a7cfbefa5dede`. It cannot accept a different run, artifact, source, extension, version, filename, or digest and never rebuilds or repackages the VSIX.

The repository automation performs these fail-closed steps:

1. `recovery-preflight` runs only on a first-attempt manual dispatch from `main` with ordinary `publish` disabled. It downloads the two original artifacts by numeric ID and checks their API identity, names, expiry, producing run, branch, and commit. It then validates the exact three-file bundle, checksum, manifest, and original attempt receipt; requires no tag, draft/public release, or Marketplace `1.0.0`; and rejects any prior recovery-decision artifact.
2. The preflight binds the non-sensitive authoritative confirmation reference, complete incident identity, current recovery run/attempt, and reviewed recovery-code commit in `marketplace-recovery-decision.json`. It uploads that decision before any job receives `id-token: write`. The fixed decision-artifact name acts as a cross-run single-use lock.
3. `recovery-marketplace` revalidates the original artifacts, decision, lock, GitHub state, and Marketplace absence. It first requests a GitHub OIDC token for the exact Marketplace audience, exchanges it for the `matheus-tecchio` trusted publisher, immediately discards the short-lived credential, and performs no upload. Only after that diagnostic succeeds may the pinned VSCE command run once against the verified original `safe-code-1.0.0.vsix` with `--oidc --no-dependencies`.
4. A nonzero or ambiguous publisher result stops the workflow. The Marketplace job has no GitHub write permission, so it cannot create a tag, release, or asset. VSCE output is stripped of control sequences, URLs, known secrets, JWTs, bearer values, and common credential forms, then capped before the sanitized stage/detail reaches the Actions log. It is never uploaded as an artifact.
5. `recovery-github` runs only after Marketplace acceptance and exact-version visibility. It targets `v1.0.0` at the recorded source commit and attaches exactly the original VSIX, checksum, and manifest. If propagation is pending, rerun only this failed job from the same recovery run; never rerun the recovery Marketplace job or the whole workflow.

The following prerequisites are external configuration and evidence, not changes made by repository automation:

- Restrict the GitHub `marketplace` environment to deployments from `main`, keep it free of secrets, and use its protected approval for the recovery publisher job.
- Configure the Marketplace trusted-publishing policy for publisher `matheus-tecchio`, repository `matheustecchio/safe-code`, workflow `publish-github-release.yml`, and environment `marketplace`. Owner status is not proof that the policy exists; contact Marketplace support if the control is unavailable.
- Obtain authoritative Marketplace-side confirmation that the 2026-09-23 invocation for `matheus-tecchio.safe-code` `1.0.0` was not accepted, created no immutable version, and cannot later complete. Version absence alone never authorizes retry.
- Immediately before approval, reconfirm in the authenticated publisher view that `1.0.0` is absent and the signed-in account is still an Owner of `matheus-tecchio`.

After the recovery implementation is independently reviewed and merged to `main`, start `Publish release` with `publish: false`, `recover_1_0_0: true`, and the non-sensitive authoritative confirmation reference. Review the persisted recovery decision, then approve the protected `marketplace` deployment only if every external prerequisite still holds. Do not dispatch this mode as a readiness probe: a successful preflight consumes the one recovery decision for the incident. If either original artifact expires on 2026-10-23, stop and design a new reviewed path; do not rebuild, retry, or weaken the provenance gate.
