...
The workflow also contains a separately reviewed, single-use recovery mode for the recorded ambiguous `1.0.0` Marketplace attempt. It accepts only the original run artifacts and digest, requires authoritative Marketplace-side non-acceptance evidence plus a protected `marketplace` approval, diagnoses OIDC trust without uploading, and fails before any GitHub release mutation on a publisher error. Version absence alone never authorizes recovery. See [Release Workflow](./docs/dev/development.md#incident-recovery-for-the-ambiguous-100-attempt) for the exact incident gate and operator sequence.
